'use strict';
import { isShellTarget, isShellCodeSymbol, extractGrepTargets, globTokens } from './symbols.mjs';
...
const ENFORCE_SEARCH = process.env.SHELL_LSP_ENFORCE_SEARCH !== 'false';     // fail-open
const ENFORCE_READ_GATE = process.env.SHELL_LSP_ENFORCE_READ_GATE === 'true'; // fail-closed

const ALLOW = {};
const deny = (reason) => ({ hookSpecificOutput: { hookEventName: 'PreToolUse', permissionDecision: 'deny', permissionDecisionReason: reason } });

const LSP_HINT = (file) =>
  `shell-lsp: use the LSP tool on ${file} (jump to definition / find references) instead of text search. ` +
...
function symbolInSearch(toolName, input) {
  if (toolName === 'Grep') return String(input.pattern ?? '').split('|').some(isShellCodeSymbol);
  if (toolName === 'Glob') return globTokens(input.pattern).some(isShellCodeSymbol);
  if (toolName === 'Bash') return extractGrepTargets(input.command).symbols.some(isShellCodeSymbol);
  return false;
}

function searchGuard(toolName, input) {
  if (!ENFORCE_SEARCH) return ALLOW;
  if (!isShellTarget(toolName, input)) return ALLOW;
  if (!symbolInSearch(toolName, input)) return ALLOW;
  return deny(LSP_HINT('the shell file you are searching'));
}
...
function readGate(cwd, file) {
  ...
  if (!s.warmupDone) {
    reason = `shell-lsp: warm up the LSP first — call the LSP tool on ${file} (jump to definition / find references), then re-read.`;
  } else if (s.readCount >= GATE2_AT && s.navCount < 2) {
    // Gate 2: >=4 reads and fewer than 2 LSP calls -> require surgical mode (2 navs)
    reason = `shell-lsp: ${s.readCount} reads with <2 LSP navigation calls. Make one more LSP call (find references / jump to definition) on ${file} to enter surgical mode.`;
  }
...
export function handlePreToolUse(event) {
  try {
    ...
    maybeReset(cwd);
    if (tool_name === 'Read') {
      if (!isShellTarget('Read', input)) return ALLOW;    // shell-scope guard: non-shell reads pass through
