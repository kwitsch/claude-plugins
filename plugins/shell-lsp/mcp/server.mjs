...
// Started via #!/usr/bin/env node, it re-execs under bun when available (SHELL_LSP_NO_BUN=1 bypasses).
...
const SERVER_NAME = "shell-lsp-hooks";
...
// SHELL_LSP_NO_BUN=1 skips re-exec (used by smoke tests for determinism).
if (process.versions.bun || process.env.SHELL_LSP_NO_BUN === "1") {
  startServer();
} else {
  ...
  const HOOK_TOOLS = [
    {
      ...
      description: "shell-lsp PreToolUse enforcement",
      inputSchema: { type: "object", additionalProperties: true },
    },
    {
      name: "hook_posttooluse",
      description: "shell-lsp PostToolUse LSP tracker",
      inputSchema: { type: "object", additionalProperties: true },
    },
  ];
