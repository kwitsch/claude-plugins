#!/usr/bin/env bash
# vtsls launcher for the js-lsp plugin.
...
if command -v bun >/dev/null 2>&1; then
  echo "js-lsp: vtsls not found; installing $PKG via bun..." >&2
  bun add -g "$PKG" >&2 || echo "js-lsp: 'bun add -g' failed; will try npx" >&2
  ...
  echo "js-lsp: bun bin path not found; falling back to npx..." >&2
fi
...
echo "js-lsp: none of vtsls, bun, or npx is available. Install Node.js (>=18) or Bun." >&2
exit 1
