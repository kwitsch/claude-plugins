...
| `memory-audit` | Discovers every CLAUDE.md and `.claude/rules/*.md` file in the project (default: whole repo), audits each against the `cc-reference` memory rules (reusing the `cc-reviewer` agent), grades them in a claude-md-improver-style quality report, recommends leanness/scope-splits (subfolder `CLAUDE.md` or `.claude/rules/` with `paths:`); then with `--fix` auto-applies every fixable finding, or without `--fix` applies only the improvements you pick via an `AskUserQuestion` multi-select. |
...
| `lsp-audit`    | Audits a project's file extensions against its project-root `.lsp.json` and additively adds the missing LSP server entries a bundled catalog recognizes — `--fix` auto-adds them, otherwise an `AskUserQuestion` multi-select picks which; extensions with no known server are reported for manual handling, never guessed.                                                                                                                                                                       |
...
| `claude-code-plugins-reference.md`             | Plugin authoring: `plugin.json` schema, layout, path variables, components, dependencies, CLI.                                                                                                 |
| `claude-code-plugins-marketplace-reference.md` | `marketplace.json` schema: top-level fields, plugin entry fields, source types, `claude plugin validate`.                                                                                      |
