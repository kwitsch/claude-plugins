...
- The combined `description` length of your non-built-in subagents is budgeted: past 15,000 tokens total, Claude Code shows a startup warning with the total token count. Trim `description` fields and move detail into each subagent's system prompt (only loaded when that subagent runs).
...
## Subagent files skipped at discovery

Applies to project/user/managed `agents/` dirs and `--add-dir` dirs — NOT silently reported in the session unless noted.

| Frontmatter problem                       | Result                                                                      |
| ----------------------------------------- | --------------------------------------------------------------------------- |
| No `name`                                 | treated as documentation kept beside agents (skipped, no error)             |
| Opening `---` isn't the file's first line | read as having no frontmatter; treated as documentation (skipped, no error) |
| `name` starts with `-` or contains `:`    | skipped; error written to debug log (`--debug` to see it)                   |
| `name` present but no `description`       | skipped; reason written to debug log                                        |
| YAML that doesn't parse                   | no fields read; skipped; parse error written to debug log                   |

- A **plugin** subagent whose frontmatter has no `name` or fails to parse still loads anyway, registered under its filename — plugin agents don't get skipped the way project/user/managed ones do.
- version >= 2.1.233: `claude plugin validate <dir>` (e.g. `.claude/agents` or `~/.claude/agents`) checks only the named directory for frontmatter that fails to parse; it does not flag a file whose frontmatter parses but has no `name`.
...
| Field             | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`            | lowercase + hyphens, unique. Hooks receive it as `agent_type`. Filename need not match. Can't start with `-` or contain `:` (`:` reserved for plugin-scoped identifiers, e.g. `my-plugin:reviewer`) — version >= 2.1.218: a file whose `name` contains `:` fails to load (error logged to debug log); earlier versions accepted it. A `name` starting with `-` is skipped the same way (see Subagent files skipped at discovery).                                             |
| `description`     | when Claude should delegate here. Add "use proactively" to encourage delegation. Budgeted: see the description-length note under Built-in subagents.                                                                                                                                                                                                                                                                                                                          |
...
| `model`           | `sonnet\|opus\|haiku\|fable`, full ID (`claude-opus-5`, `claude-sonnet-5` — same values as `--model`), or `inherit`. Default: picked per the Model resolution order below.                                                                                                                                                                                                                                                                                                    |
...
| `maxTurns`        | max agentic turns before stop. version >= 2.1.246: on hitting the limit, the returned output is marked partial and Claude can resume the subagent to continue; earlier versions returned the output with no partial marking.                                                                                                                                                                                                                                                  |
...
- version >= 2.1.251: 1. per-invocation `model` param → 2. definition `model` frontmatter (`inherit` selects the main conversation's model) → 3. `CLAUDE_CODE_SUBAGENT_MODEL` env var (only when set to a model alias or ID) → 4. main conversation model.
- version <= 2.1.250: `CLAUDE_CODE_SUBAGENT_MODEL` came FIRST and overrode both the per-invocation param and the frontmatter, including `model: inherit`.
- `CLAUDE_CODE_SUBAGENT_MODEL` alone never changes what the built-in Explore/Plan subagents run on (their own resolution stays as described under Built-in subagents).
- version >= 2.1.196: `CLAUDE_CODE_SUBAGENT_MODEL=inherit` is treated as unset — resolution falls through the rest of the order. Earlier versions: `inherit` forced the main conversation's model and skipped every other source.
...
- In a session where the Task tools are unavailable, Claude Code doesn't provide them to subagents either, even if the subagent runs a different model. An in-process teammate follows the session's tool availability; a teammate in its own split pane runs as a separate process so its own model decides.
...
- `mcpServers:` entries = inline def (`stdio|http|sse|ws`, same schema as `.mcp.json`) connected for the agent's lifetime, OR a string name reusing the parent's connection. Applies whether the agent file runs as a subagent or as the main session (`--agent`/`agent` setting) — in the main-session case inline defs connect at startup alongside `.mcp.json` + settings-file servers, under the same trust rule below.
- Define an MCP server inline here (not in `.mcp.json`) to keep its tool descriptions OUT of the main conversation's context.
- version >= 2.1.238: an inline `mcpServers` entry in an agent file from a project's `.claude/agents/`, or from an `--add-dir` dir's `.claude/agents/`, loads only after you trust the folder the agent file came from (a parent folder's trust, or the automatic trust a `-p`/SDK session gets for settings-file hooks, does NOT count). Until trusted: Claude Code skips every inline server in that file and logs the exact `projects["<path>"].hasTrustDialogAccepted` key to check, to the debug log. An `--add-dir` folder outside your trusted workspace's repo needs its own trust entry — it doesn't inherit the workspace's grant. Before v2.1.238: these servers loaded without a trust check. Loads WITHOUT a trust check regardless of version: a string name referencing an already-configured server; an inline server in `~/.claude/agents/`, in one passed via `--agents`/SDK `agents`, or in one managed settings supplies.
...
- **In frontmatter** (run only while this agent active; cleaned up on finish): all hook events. Common: `PreToolUse`, `PostToolUse`, `Stop` (auto-converted to `SubagentStop` when run as a subagent). Fire both when spawned as subagent and when run as main via `--agent`; in the main-session case they run alongside any `settings.json` hooks. version >= 2.1.218: project-level subagent frontmatter hooks require workspace trust for the folder containing the agent file — without it the subagent still runs but its hooks are skipped (error in debug log); user-level (`~/.claude/agents/`) and `--agents`-defined hooks run without this step. Before v2.1.218, frontmatter hooks ran without workspace trust.
...
- **Named subagent + agent teams:** in an interactive session with agent teams enabled, a subagent Claude spawns with a `name` parameter from the main conversation launches as a teammate instead, UNLESS the call is a fork or passes `isolation` on the call itself. An `isolation` value in the subagent's frontmatter does not prevent it.
...
- An in-process agent-team teammate that spawns a subagent always runs it in the foreground and refuses `background: true` definitions with an error; where fork mode is off and background tasks aren't disabled, it also refuses with an error when the teammate requests `run_in_background: true`.
...
- A configured fallback model chain overrides this for failures the chain covers (e.g. the subagent's model becoming unavailable): Claude Code switches the subagent to the first chain model that accepts the request and it keeps working, instead of ending on the error.
...
- version >= 2.1.232: fork mode is ON by default in interactive sessions. Before v2.1.232: fork mode was off by default — set `CLAUDE_CODE_FORK_SUBAGENT=1` to enable it. Fork mode remains OFF by default in non-interactive mode (`-p`) and in the Agent SDK. `CLAUDE_CODE_FORK_SUBAGENT=1` forces it on in any session type; `=0` forces it off everywhere.
- When fork mode is on: Claude forks only by requesting the `fork` subagent type explicitly; untyped spawns still use `general-purpose` and named subagents (e.g. Explore) spawn as before. Every subagent spawn — fork or named — runs in background while fork mode is on (`CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1` takes precedence and keeps subagents synchronous); frontmatter `background` has no effect in this mode since fork mode removes the Agent tool's `run_in_background` param.
...
- **Manual: `/subtask <directive>`** (version >= 2.1.212; works with or without fork mode; Claude Code names the fork from the first words of the task). `/fork` copies the whole session into a new background session with its own budget — EXCEPT when agent view is turned off, where `/subtask` is unavailable and `/fork` starts the forked subagent instead.
- version 2.1.161–2.1.211: the forked-subagent command is `/fork`, not default-on in interactive sessions (still required `CLAUDE_CODE_FORK_SUBAGENT=1`). version 2.1.117–2.1.160: `/fork` required `CLAUDE_CODE_FORK_SUBAGENT=1`.
...
- An Agent tool call with no `subagent_type` gets the built-in `general-purpose` subagent by default. With `CLAUDE_AGENT_SDK_DISABLE_BUILTIN_AGENTS=1` that default is gone too, so the call fails with `subagent_type is required: the general-purpose agent is not available in this session` (message ends with the types still available). version >= TS SDK 0.3.235 / Python SDK bundling Claude Code >= v2.1.235: that error text. Earlier: the same call failed with `Agent type 'general-purpose' not found`.
...
- v2.1.232 fork mode is on by default in interactive sessions (was: required `CLAUDE_CODE_FORK_SUBAGENT=1`); a successful background subagent's panel row is removed immediately with a 30 s footer hint instead of keeping the row for 30 s; same change applies to finished fork rows.
