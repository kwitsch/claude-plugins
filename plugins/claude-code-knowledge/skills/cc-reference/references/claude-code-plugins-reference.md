...
| `plugin update`             | `claude plugin update <plugin> [-y] [-s scope]`                                                                           | Scope may be `user`/`project`/`local`/`managed`. `-y`/`--yes` accepts a marketplace-declared command without confirmation — a `command`-source plugin's command, or (version >= 2.1.238) an archive `headersHelper`; required when stdin/stdout is not a TTY; no effect inside a Claude Code session. version >= 2.1.246: a bare name ambiguous across installed marketplaces is refused, listing the qualified `plugin-name@marketplace-name` commands to run instead (earlier: bare name rejected outright as not found).                                                                                        |
