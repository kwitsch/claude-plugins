...
- Orphaned version directories (after update/uninstall) are auto-deleted after 7 days; Glob/Grep skip them. Grace period lets concurrent sessions on the old version keep running.
- `CLAUDE.md` at plugin root is NOT loaded as project context — ship instructions via a skill.
- Single-skill plugin: a `SKILL.md` at plugin root (no `skills/` dir, no `skills` manifest field) loads as one skill (version >= 2.1.142). Invocation name = frontmatter `name`, else directory basename.
...
- **Path-behavior rule**: REPLACE fields skip the default dir entirely; to keep it, list it explicitly (e.g. `"commands": ["./commands/", "./extras/"]`). `skills` always ADDS. Exception: a marketplace entry whose `source` resolves to the marketplace root — declaring specific skill subdirs REPLACES the default scan.
- All custom paths must be relative, start with `./`, resolve from plugin root.
- version >= 2.1.140: when both a default folder and its manifest key exist, the ignored folder is flagged in `/doctor`, `plugin list`, `/plugin` detail (plugin still loads via manifest paths).
...
- `claude plugin validate` reports unrecognized fields as **warnings** (with a suggestion if 1-2 chars off a known field); wrong-type fields (e.g. `keywords` as string) are load **errors**.
...
- Substituted as `${user_config.KEY}` in MCP/LSP configs, hook commands, monitor commands; non-sensitive also in skill/agent content.
- All values exported to plugin subprocesses as `CLAUDE_PLUGIN_OPTION_<KEY>` env vars.
...
|---|---|---|
| `${CLAUDE_PLUGIN_ROOT}` | per-version | Plugin install directory. Changes on each plugin update; old dir kept ~7 days then cleaned. Ephemeral — do NOT write state here. Use for bundled scripts/binaries. |
...
- All three are substituted inline in skill/agent content, hook commands, monitor commands, MCP/LSP configs; all also exported as env vars to hook + MCP/LSP subprocesses.
...
- Plugin hooks/commands additionally substitute `${user_config.*}` and any `${ENV_VAR}`.
...
An entry may carry ANY field from the [plugin.json schema](#standard-metadata-fields) (`description`, `version`, `author`, `homepage`, `repository`, `license`, `keywords`, `displayName`, `defaultEnabled`, plus component-path fields `skills`/`commands`/`agents`/`hooks`/`mcpServers`/`lspServers`) plus these marketplace-specific fields:
...
- For git sources (`github`/`url`/`git-subdir`), when both `ref` and `sha` are set, `sha` is the effective pin — install succeeds even if the `ref` branch/tag was deleted upstream, as long as the commit is reachable. `sha` = full 40-char commit SHA.
...
- Explicit `version` pins: must bump on every release; pushing commits without bumping does nothing (cache kept, `/plugin update` reports already-latest). Avoid setting in BOTH `plugin.json` and marketplace entry — `plugin.json` wins silently.
...
4. Yes → plugin installed to user scope. "No, and don't show again" disables all future hint prompts. No-response within 30 s dismisses as No. At most once per plugin and once per session (across all CLIs).
...
|---|---|---|
| `plugin init` | `claude plugin init <name> [--with <components…>] [-f]` | Scaffold at `~/.claude/skills/<name>/`; loads next session as `<name>@skills-dir`. `--with`: `skills agents hooks mcp lsp output-style channel`. Alias `new`. Blocked when `skills-dir` source is in managed `blockedMarketplaces`/excluded by `strictKnownMarketplaces`. |
| `plugin install` | `claude plugin install <plugin> [-s scope]` | Install from marketplace; `<plugin>` = `name` or `name@marketplace`; default scope `user`. Resolves + installs declared dependencies. |
| `plugin uninstall` | `claude plugin uninstall <plugin> [--keep-data] [--prune] [-y] [-s scope]` | Remove; data dir deleted from last scope unless `--keep-data`. `--prune` also removes orphaned auto-deps. Aliases `remove`, `rm`. |
...
| `plugin disable` | `claude plugin disable <plugin> [-s scope]` | Disable without uninstalling; fails if another enabled plugin depends on target (error gives chained command). |
...
| `GITHUB_TOKEN`/`GH_TOKEN`, `GITLAB_TOKEN`/`GL_TOKEN`, `BITBUCKET_TOKEN` | Auth for background auto-update of private-repo marketplaces (manual install/update uses git credential helpers + ssh-agent). |
