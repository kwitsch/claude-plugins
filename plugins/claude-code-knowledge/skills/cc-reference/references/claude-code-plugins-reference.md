...
- Plugin agent frontmatter with no `name` field, or that fails to parse, still loads: Claude Code names the agent after the file (`agents/reviewer.md` in plugin `my-plugin` → `my-plugin:reviewer`); unparseable frontmatter additionally gets description `Agent from my-plugin plugin` and ignores every other field in the file. A project/user/managed agent file with the same defect is skipped instead. Find these with `claude plugin validate ./my-plugin/agents` (bare agents dir; version >= 2.1.233) or `claude plugin validate ./my-plugin` (plugin with a manifest).
...
| ---------------- | ------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`           | string  | Yes      | Unique identifier; kebab-case, no spaces, control characters, or bidirectional-formatting characters (version >= 2.1.247: reported as its own validation error rather than the impersonation-name error). Used for component namespacing. If a marketplace entry lists the plugin under a different `name`, the marketplace entry's name is authoritative for `enabledPlugins` keys and `/plugin`. |
...
| `experimental.evals`    | string \| array           | Directory below the plugin root holding `claude plugin eval` case files, when not the default `evals/`; `claude plugin eval --eval-dir` overrides. version >= 2.1.269                                                                      | REPLACES default `evals/`                 |
...
- On mid-session plugin update, hooks/monitors/MCP/LSP keep the old path; `/reload-plugins` switches hooks/MCP/LSP to the new path, monitors need a session restart. In a session with no interactive terminal, `/reload-plugins` leaves plugin MCP servers on the old path until the next session. `/reload-plugins` also keeps live connections of MCP servers whose configuration is unchanged (reconnects only changed servers).
...
| `bin/`                   | PATH executables (invokable as bare commands in Bash tool while enabled); can't be included in a plugin distributed through claude.ai org settings (rejected on sync or direct upload) |
...
A marketplace is a `marketplace.json` catalog that lets users browse and install plugins via `/plugin` or CLI. Full schema (required/optional top-level fields, plugin entry fields, source types, validate): see `references/claude-code-plugins-marketplace-reference.md`.
...
- Local dev: `claude --plugin-dir ./dep --plugin-dir ./plugin` satisfies a dependency entry with the local copy even when the entry names a marketplace, and skips checking any version constraint against it (version >= 2.1.242; earlier, a marketplace-qualified entry never matched the local copy and the plugin failed to load). Without also installing the dependency from its marketplace: disabling the local copy reports `Dependency "<name>@inline" is disabled — enable it or remove the dependency` (a bare-name entry reports the bare name); dropping the `--plugin-dir` flag reports the dependency as not installed. When both plugins sit under one parent folder, pass that folder to `--plugin-dir` once instead of repeating the flag (version >= 2.1.265; see [Session flags](#session-flags)).

## Synced plugins (Cowork / cloud sessions)

- Cowork and cloud sessions download the plugins enabled for the user's claude.ai account into `~/.claude/plugins/synced/` and load each as `<name>@synced` — no marketplace, no install record. Not loaded in a session started directly in a terminal. version >= 2.1.239 (earlier: loaded as `<name>@inline`, the `--plugin-dir`/`--plugin-url` identity).
- `claude plugin list` shows these under a `Synced from claude.ai` heading in that environment; they do not appear in the inline `/plugin list`.
- Manage by the `<name>@synced` id: `claude plugin disable`/`enable <name>@synced` in-session writes `"<name>@synced": false`/`true` to that environment's user-level `enabledPlugins`. `install`/`update`/`uninstall` don't apply to a synced plugin — turn it off for the claude.ai account instead, or set `"<name>@synced": false` under `enabledPlugins` in a project's committed `.claude/settings.json` to keep it out of that project's synced sessions everywhere.
- An enabled plugin from any other source (marketplace, `@skills-dir`, `--plugin-dir`) that shares a synced plugin's name wins; the synced copy reports as not loaded. Disable your own copy to use the claude.ai one instead. Before version 2.1.239, the synced copy won this conflict instead.
...
- Project-scope `@skills-dir` plugins load only from the `.claude/skills/` of the session's primary working directory — they do NOT walk up to repo root, so launching from a subdirectory misses a plugin at the repo root. Launch from repo root, or (version >= 2.1.246) move the session there with `/cd` — a plain shell `cd` does not change which directory is scanned.
...
- **`command` source** version is always derived from the command's output: a 12-character content hash alone, or `<version>-<hash>` when `plugin.json` sets `version`. The marketplace entry's `version` field is ignored for `command` sources. In link mode the hash covers the printed directory's real path and its top-level entries (not file contents). Any change in hashed output = new version, even with an unchanged `version` string.
...
4. Yes → plugin installed to user scope. "No, and don't show again" disables all future hint prompts. No-response within 30 s dismisses as No. At most once per plugin and once per session (across all CLIs), and only in the main interactive session the user is typing into — never for a command a subagent runs, and never in a `-p`/Agent-SDK session (the hint line is still stripped in both cases). Sessions with telemetry disabled never prompt — `DISABLE_TELEMETRY`, `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC`, or a third-party provider (Amazon Bedrock, Google Cloud Agent Platform) where the automatic telemetry opt-out applies.
...
| `plugin install`            | `claude plugin install <plugin> [--config <key=value>] [-y] [-s scope] [--json]`                                                                                                                                                                                                                 | Install from marketplace; `<plugin>` = `name` or `name@marketplace`; scope `user`/`project`/`local`, default `user`. Resolves + installs declared dependencies. `--config` sets a declared `userConfig` option (repeat the flag for several). `-y`/`--yes` accepts a marketplace-declared command without the confirmation prompt — a `command`-source plugin's directory-producing command, or (version >= 2.1.238) an archive `headersHelper` — Claude Code still prints the command first; required when stdin/stdout is not a TTY; has no effect inside a Claude Code session (run it from your own terminal). `--json` (version >= 2.1.268): last stdout line is one JSON object — always `command`, `outcome` (`ok`/`failed`), `message`; `pluginId`/`scope`/`failureCode` when they apply. A usage error (e.g. bad `--scope`) prints no result line, exits 1, reason on stderr. Same `--json` shape on `uninstall`/`update`/`enable`/`disable` below. |
| `plugin uninstall`          | `claude plugin uninstall <plugin> [--keep-data] [--prune] [-y] [-s scope] [--json]`                                                                                                                                                                                                              | Remove; data dir deleted from last scope unless `--keep-data`. `--prune` also removes orphaned auto-deps; `-y` skips its confirmation (required when stdin/stdout is not a TTY). Aliases `remove`, `rm`. version >= 2.1.212: the `name@marketplace` form uninstalls only from the named marketplace (earlier versions could match a same-named plugin from a different one). `--json` (version >= 2.1.268) can't combine with `--prune`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `plugin enable`             | `claude plugin enable <plugin> [-s scope] [--json]`                                                                                                                                                                                                                                              | Enable; enables dependencies transitively (fails if a dep is not installed). `--json` version >= 2.1.268.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `plugin disable`            | `claude plugin disable [plugin] [-a] [-s scope] [--json]`                                                                                                                                                                                                                                        | Disable without uninstalling; fails if another enabled plugin depends on target (error gives chained command). `-a`/`--all` disables every enabled plugin and cannot be combined with `--scope`; `[plugin]` is optional only with `--all`. `--json` version >= 2.1.268.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `plugin update`             | `claude plugin update <plugin> [-y] [-s scope] [--json]`                                                                                                                                                                                                                                         | Scope may be `user`/`project`/`local`/`managed`. `-y`/`--yes` accepts a marketplace-declared command without confirmation — a `command`-source plugin's command, or (version >= 2.1.238) an archive `headersHelper`; required when stdin/stdout is not a TTY; no effect inside a Claude Code session. version >= 2.1.246: a bare name ambiguous across installed marketplaces is refused, listing the qualified `plugin-name@marketplace-name` commands to run instead (earlier: bare name rejected outright as not found). `--json` version >= 2.1.268.                                                                                                                                                                                                                                                                                                                                                                                                     |
| `plugin list`               | `claude plugin list [--json] [--available]`                                                                                                                                                                                                                                                      | `--available` needs `--json`. `/plugin list` inline supports `--enabled`/`--disabled`; alias `ls`. `@skills-dir` plugins show in `/plugin` + `claude plugin list` but NOT in inline `/plugin list`; `--plugin-dir`/`--plugin-url` plugins only when the flag precedes the subcommand (`claude --plugin-dir <dir> plugin list`). A row with load problems carries `errors`/`notes` string arrays; version >= 2.1.268 adds parallel `errorDetails`/`noteDetails` arrays with each diagnostic's `type` and the names it refers to (plugin, marketplace, server, file).                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `plugin validate`           | `claude plugin validate <path> [--strict] [--json]`                                                                                                                                                                                                                                              | Validate marketplace or plugin JSON; `--strict` = warnings as errors. Exit 0 pass / 1 fail / 2 validation-run failure itself (e.g. unreadable path — nothing on stdout, error to stderr). `--json` (version >= 2.1.259) emits one JSON object instead of text: `success`, `strict`, `target`, `manifest` (`null` for a manifest-less run), `contents` (per-file `file`/`errors`/`warnings`/`notes`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `plugin eval`               | `claude plugin eval [target] [--runs n] [-j/--concurrency 1-8] [--model m] [--judge-model m] [--ablation none\|with-without] [--threshold 0..1] [--max-cost-usd usd] [--allow-tools tools…] [--scaffold] [--trust-plugin] [--mocks record\|off] [--eval-dir dir] [--json [path]] [--no-publish]` | version >= 2.1.269. Runs a plugin's eval cases (prompt + graders) several times each, isolated, with/without the plugin loaded (`--ablation with-without` default when a plugin resolves) to show what it changes. `target` = plugin dir, a `prompt.md`/`case.yaml` file, or installed `name`/`name@marketplace`/`name@skills-dir`; defaults to cwd. Cases directory defaults to the manifest's `experimental.evals`, else `evals/`. Exit 0 every case ≥ threshold / 1 a failing case, load error, or untrusted plugin dir / 2 partial run (e.g. `--max-cost-usd` hit) / 130 interrupted / 143 terminated.                                                                                                                                                                                                                                                                                                                                                   |
| `plugin eval init`          | `claude plugin eval init [name] [--bare] [-i/--interactive] [--eval-dir dir]`                                                                                                                                                                                                                    | version >= 2.1.269. Scaffolds an eval suite for the plugin in the cwd. In a terminal, runs an authoring interview (reads the plugin, proposes + pilots cases/graders, writes files); `--bare`, or no terminal, writes a blank single-case template instead (needs `name`); `-i`/`--interactive` requires the interview and fails without a terminal rather than falling back. Run from inside a Claude Code session, it prints the interview instructions for that session to follow instead of writing files.                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| `plugin marketplace update` | `claude plugin marketplace update [name]`                                                                                                                                                                                                                                                        | Refresh from sources (a `ref`-pinned marketplace updates to that ref's latest commit, not the repo's default branch); seed-managed entries skipped.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- `claude --plugin-dir <folder>` — version >= 2.1.265: when `<folder>` itself has no manifest or plugin components at its top level, each immediate subfolder with a `.claude-plugin/plugin.json` loads as a separate plugin (everything else in the folder, including manifest-less subfolders, is skipped without error). In an interactive session Claude Code also watches the folder: adding/removing a subfolder loads/unloads its plugin live, or — when applying the change mid-conversation would invalidate the prompt cache — prints a line telling you to run `/reload-plugins`.
...
- version >= 2.1.205: LSP `restartOnCrash` and `shutdownTimeout` fields recognized; before 2.1.205 the schema accepted both but setting either caused Claude Code to skip that LSP server at startup (reason visible only in `claude --debug`). Reserved marketplace names re-checked on every marketplace load; `first-party-plugins`/`healthcare` added to the reserved list.
...
- version >= 2.1.229: `command` marketplace plugin-source type (plugin directory produced by a local command, re-run once per session — see [Version resolution](#version-resolution)).
- version >= 2.1.232: `blockedMarketplaces` `url` entries also match a bare cloned `https://` repo add, not only a fetched `marketplace.json` URL.
- version >= 2.1.233: `claude plugin validate` can target a specific `skills`/`agents`/`commands` directory (or a manifest-less plugin dir) to find skill/agent/command frontmatter parse errors, not only `plugin.json`/`marketplace.json` schema errors.
- version >= 2.1.238: `claude plugin install`/`update -y` also accepts an archive `headersHelper` command.
- version >= 2.1.239: plugins synced from claude.ai load as `<name>@synced` in Cowork/cloud sessions (earlier: `<name>@inline`, the `--plugin-dir`/`--plugin-url` identity) — see [Synced plugins](#synced-plugins-cowork--cloud-sessions).
- version >= 2.1.242: a `--plugin-dir` dependency entry that names a marketplace matches the local copy (earlier: never matched, dependent plugin disabled at load).
- version >= 2.1.246: `claude plugin update <bare-name>` ambiguous across marketplaces is refused with qualified commands to run instead (earlier: rejected outright as not found); project-scope `@skills-dir` plugins can move the session with `/cd` instead of relaunching from repo root.
- version >= 2.1.247: plugin/marketplace `name` rejects control characters and bidirectional-formatting characters as its own validation error (earlier: reported as the marketplace-impersonation error instead, or not checked at all).
- version >= 2.1.259: `claude plugin validate --json` emits the validation report as one JSON object (`success`, `strict`, `target`, `manifest`, `contents`).
- version >= 2.1.265: `--plugin-dir <folder>` loads every immediate subfolder holding a `.claude-plugin/plugin.json` as a separate plugin, when the named folder itself has no manifest or components at its top level; watched live in interactive sessions.
- version >= 2.1.268: `plugin install`/`uninstall`/`update`/`enable`/`disable --json` emit one unified JSON result object (`command`, `outcome`, `message`, plus fields that apply, e.g. `pluginId`/`scope`/`failureCode`) on the last stdout line; `plugin list --json` rows gain parallel `errorDetails`/`noteDetails` arrays alongside `errors`/`notes`.
- version >= 2.1.269: `claude plugin eval` / `claude plugin eval init` — run and author a plugin's eval-case suite (prompt + graders, scored per case, optional no-plugin baseline ablation); `experimental.evals` plugin.json field names the case directory when it isn't the default `evals/`.
