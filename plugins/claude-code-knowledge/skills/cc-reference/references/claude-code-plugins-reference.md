...
- On mid-session plugin update, hooks/monitors/MCP/LSP keep the old path; `/reload-plugins` switches hooks/MCP/LSP to the new path, monitors need a session restart. `/reload-plugins` also keeps live connections of MCP servers whose configuration is unchanged (reconnects only changed servers).
...
- An enabled plugin from any other source (marketplace, `@skills-dir`, `--plugin-dir`) that shares a synced plugin's name wins; the synced copy reports as not loaded. Disable your own copy to use the claude.ai one instead.
