...
| --------- | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`    | string | Marketplace identifier (kebab-case); public-facing; no control or bidirectional-formatting characters (own validate error version >= 2.1.247, earlier versions reported the impersonation error instead). One marketplace per name per user (re-adding same name replaces). Reserved names blocked (see below). |
...
| `metadata.pluginRoot`                 | string | version >= 2.1.239. Base dir prepended to a bare-name relative plugin source (e.g. `"./plugins"` lets you write `"source": "formatter"` for `"./plugins/formatter"`); must itself be a relative path inside the marketplace. Ignored for a source already starting with `./`; a source containing `/` (e.g. `team-a/formatter`) isn't a bare name and still needs the `./` prefix even when this is set.                                                                                                                                                                                                                                                |
...
| Field            | Type           | Description                                                                                                                                                                                                                        |
| ---------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `name`           | string         | Plugin identifier (kebab-case). Required. No control or bidirectional-formatting characters (version >= 2.1.247; earlier versions ran no such check).                                                                              |
...
**Archive authentication fields** (set beside `source` for an `archive` entry on a server that needs credentials; version >= 2.1.238):

| Field           | Type   | Description                                                                                                                                                                                                                                                                                                      |
| --------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `headers`       | object | HTTP headers sent with this entry's archive download. Overrides a same-named header set on the marketplace's `url` source.                                                                                                                                                                                       |
| `headersHelper` | string | Command printing a JSON headers object for a short-lived credential. Entry must also set `strict: false`. Runs only when a user installs/updates that one plugin by itself and accepts the command (`--yes` non-interactive); never for a bulk install, a suggestion install, or as another plugin's dependency. |

`headersHelper` command contract: printable ASCII ≤500 chars, no run of ≥4 spaces; prints one JSON object on stdout and exits 0 within 10 s, or Claude Code skips the fetch/download it was for. Runs via `sh`/`cmd.exe` from `~/.claude` (or `CLAUDE_CONFIG_DIR`) — use an absolute path or a `PATH` command, not a project-relative one. Sets `CLAUDE_CODE_PLUGIN_NAME`/`CLAUDE_CODE_PLUGIN_ARCHIVE_URL` in the entry-level command's env (a marketplace `url`-source command instead gets `CLAUDE_CODE_MARKETPLACE_URL`/`CLAUDE_CODE_MARKETPLACE_NAME`, the latter unset on the first fetch after adding by URL); strips env vars named `*TOKEN*`/`*SECRET*`/`*KEY*`/`*AUTH*` for a command declared in `marketplace.json` or in project settings (not user/managed settings). Claude Code drops the headers/skips the command on: command failure; a marketplace URL not starting with `https://`; a redirect that leaves the archive URL's origin (drops both places' headers); routing/identity header names (`Host`, `Cookie`, `X-Forwarded-*`); a command declared in an `--add-dir` directory's settings; or `disableCommandPluginSources`/`allowManagedHooksOnly`. Before v2.1.238: entry-level `headers`/`headersHelper` were never sent — install fails with `HTTP 401 while downloading plugin archive from <url>`.
...
### Test a dependency locally with --plugin-dir

`claude --plugin-dir ./my-dependency --plugin-dir ./my-plugin` — the local copy satisfies the dependency entry even when the entry names a marketplace; no version constraint is checked against a local copy, so its `plugin.json` needs no `version`. version >= 2.1.242 required for a marketplace-naming entry to match the local copy this way (earlier: never matched, plugin disabled at load). Passing one parent folder that holds both plugins (each with its own `.claude-plugin/plugin.json`) loads each child the same way — version >= 2.1.265.

If the local copy later goes away: a disabled local copy reports `Dependency "<name>@inline" is disabled — enable it or remove the dependency` for a marketplace-naming entry (a bare-name entry is reported by its bare name); dropping the `--plugin-dir` flag entirely reports the dependency as not installed.
...
- version >= 2.1.233: `claude plugin validate <dir>` also runs against a directory with no manifest at all — a bare `skills`/`agents`/`commands` directory, under a plugin, under `.claude`, or under `~/.claude` — reporting a YAML/JSON parse error per file; doesn't follow symlinks into or under it (warns instead of reading).

## Managed marketplace restrictions

`strictKnownMarketplaces` in managed settings restricts which marketplaces users can add: undefined = unrestricted; empty array `[]` = block every marketplace, including the official Anthropic one; a list = allowlist enforced. Supported source types for entries:
...
| `CLAUDE_CODE_MARKETPLACE_URL` / `CLAUDE_CODE_MARKETPLACE_NAME` | version >= 2.1.238. Set in the env of a marketplace `url` source's `headersHelper` command. `NAME` is unset on the first fetch after a user adds the marketplace by URL.                                       |
| `CLAUDE_CODE_PLUGIN_NAME` / `CLAUDE_CODE_PLUGIN_ARCHIVE_URL`   | version >= 2.1.238. Set in the env of a plugin entry's `headersHelper` command.                                                                                                                                |
...
- **Main interactive session only**: shown only in the terminal the user is typing into — never for a command a subagent runs, and never in non-interactive `-p` mode or through the Agent SDK (the hint line is still stripped from the output in all these cases).
