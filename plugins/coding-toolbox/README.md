...
| `finish-pr`           | Finalize an existing PR/MR for the current branch before merge — abort if none exists, rebase it onto its base and force-push if the base has moved ahead, undraft it, enable GitLab's delete-source-branch-on-merge if off, and reconcile its title/description against the actual diff.                                                                                                                                                                                                                                                                                 |
...
| `bump-version`        | Bump a project's semantic version (major/minor/patch) in its detected version file (`.claude-plugin/plugin.json` — a Claude Code plugin, e.g. inside a plugin-marketplace repo — then `package.json`, `composer.json`, `pom.xml`, or `VERSION`) and sync the matching lock file (`npm`/`composer`) when present. No git operations.                                                                                                                                                                                                                                       |
...
| `dispatch-agent`      | Hand off an independent task to a new, worktree-isolated background Claude Code session (`claude --worktree ... --bg`, managed via `claude agents`) started from the repo's default branch on origin (or this session's `HEAD` when `worktree.baseRef` is `"head"`), passing through the given prompt. Accepts optional `--model=`/`--effort=` overrides (default `sonnet`/`xhigh`); always runs with `--permission-mode auto`.                                                                                                                                           |
