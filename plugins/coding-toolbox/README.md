...
| `setup-rules` | Install, refresh, or remove coding-toolbox's project-level rules — a golden-rules copy and a tool-routing table for `rtk`/`bun`/`rg`/`codebase-memory-mcp` — as always-on `.claude/rules/coding-toolbox-*.md` files, via one action-framed `AskUserQuestion` (no end-state toggle: `AskUserQuestion` can't pre-check boxes). |
