...
| `fresh-work` | Classify a unit of work (fix/refactor/feature) from a one-line description, cut a branch via `fresh-branch`, dispatch to the matching pipeline skill (`debugging`, `feature-development`, or `refactoring`), then finish via `fresh-pr`. Fully self-contained. |
| `feature-development` | Fresh-work's design-path pipeline: design + plan as session temp files (always self-reviewed; consult an advisor on their own judgment call), an `AskUserQuestion` intent-confirmation checkpoint between them, implement task-by-task via workflow-driven development (Workflow tool, Agent fallback), a combined verified review workflow (correctness angles + per-lens cleanup finders, effort scaled to diff complexity) before returning to `fresh-work`. Invoked for `feature` work directly, and for `refactor` work via `refactoring`'s delegation. |
...
| `refactoring` | Thin delegator to `feature-development` — `refactor` and `feature` classifications share an identical pipeline; only the branch prefix differs. Invoked by `fresh-work` for `refactor`-classified work. |
