...
PreToolUse hook (`hooks/deny-coauthor.sh`, matcher `Bash`) scans `git commit` commands. **Deny** when message carries co-author trailer or Claude Code footer. Scan only — never rewrite command.

Predates userConfig rule — no feature toggles by design (hook-only plugin, simple scope).

Matched strings: `Co-Authored-By:` (case-insensitive) and `Generated with [Claude Code](http` (full signature).

## Behavior
- Pure shell `case` match on raw hook payload; no `jq`/`node` dep. `Co-Authored-By:` and `Generated with [Claude Code](http` plain ASCII, JSON never escape, match verbatim in payload.
