...
## Behavior
- JSON read/emit prefers `jq`, falls back to `node`; if neither exists it returns a `deny` decision telling Claude to recreate the message without those lines.
- Cleaning (sed): own-line `Co-Authored-By:`, inline `-m "Co-Authored-By: …"` args, and the footer matched by its full `[Claude Code](http…` signature (so prose mentioning the footer survives).
