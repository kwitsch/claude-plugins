...
| `ponytailReview`         | `{findings: [{file, line?, tag, what, replacement}], verdict}` — a separate, report-only over-engineering ("lean") pass over the same review diff; tags `delete`/`stdlib`/`native`/`yagni`/`shrink`. NEVER auto-applied and never escalated; surfaced in the report and the PR body only                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
- The Review phase ends with a separate, report-only **lean review**
  (ponytail): an independent `sonnet` pass over the same review diff that
  reports over-engineering ONLY (reinvented stdlib, one-caller abstractions,
  dead flexibility, shrinkable code — tags `delete`/`stdlib`/`native`/`yagni`/
  `shrink`). It is distinct from and non-overlapping with the combined
  correctness+cleanup review: it reviews HOW the change was built, never
  WHETHER an approved feature should exist, and never re-reports
  correctness/security/performance. Findings return in `ponytailReview` and
  are never routed to fix application, never escalated, and add no
  `AskUserQuestion` — run `/simplify` or `ponytail-review` afterward to act
  on them.
