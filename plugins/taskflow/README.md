...
Static role prompts (rules, lens catalog, verdict ladder, git procedures), dispatched by the workflows via `agentType`; models pinned in frontmatter.

| Agent             | Model           | Role                                                                                   |
| ----------------- | --------------- | -------------------------------------------------------------------------------------- |
| `designer`        | opus (pinned)   | Writes/revises the design draft — approach, trade-offs, decisions.                     |
| `design-reviewer` | sonnet          | Read-only: placeholders, consistency, scope, ambiguity, question validation.           |
| `planner`         | opus (pinned)   | Turns an approved spec into a dense, machine-executable implementation plan.           |
...
| `fix-applier`     | sonnet (pinned) | Applies pre-verified review fixes by category, with a test-run safety gate.            |
| `pr-author`       | sonnet (pinned) | Writes the PR/MR title and body from the pipeline summary and repo template.           |
