...
- **Aggregated hooks** (`hooks/hooks.json`): mid-session PreToolUse/PostToolUse are `mcp_tool` hooks that delegate to `hook_*` tools on the same server, fanning out to both caveman and context-mode logic in one round-trip. SessionStart, UserPromptSubmit, and PreCompact are `command` hooks (each launched through `bin/mjsx.sh` with the `hooks/*.mjs` script as its argument) — each for a specific reason, not a blanket "too early": SessionStart is **pre-connect** (server not up on first run, so `mcp_tool` would fail open); UserPromptSubmit avoids a per-prompt MCP round-trip; PreCompact is a **fail-open-sensitive side-effect** (a pre-context-loss snapshot) that must fire even if the server is momentarily down. SessionStart emits the caveman ruleset as `additionalContext`; UserPromptSubmit re-emits the per-turn reminder.
