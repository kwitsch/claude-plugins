...
The MCP server (`mcp/server.mjs`) and the two SessionStart command hooks (`hooks/sessionresume.mjs`, `hooks/sessionstartup.mjs`) are invoked **directly** as the `command` — each carries a `#!/usr/bin/env node` shebang and is executable (`100755`). There is no launcher wrapper. cave-context is node-only; context-mode is vendored and run in-process, so no runtime selection or npm-package launch is needed. Node is assumed to be on the spawn `PATH`.
