...
- `hooks/hooks.json`: SessionStart = `command` hook (`hooks/sessionstart.mjs` via `mjsx.sh`) — seeds runtime state `writeLevel(stateDir(), configuredDefaultLevel())` so per-turn reminders fire from turn 1, then emits the caveman ruleset as `additionalContext`; UserPromptSubmit + PreCompact = `command` hooks (`hooks/*.mjs` via `mjsx.sh`) — `mcp_tool` fails open on early-lifecycle events because the server is not reliably connected yet; PreToolUse + PostToolUse = `mcp_tool` → `hook_*` (mid-loop, server connected).
