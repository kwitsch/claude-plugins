...
const LSP_HINT = (file) =>
  `ts-lsp: use the LSP tool on ${file} (jump to definition / find references) instead of text search. ` +
...
function readGate(cwd, file) {
  ...
  if (!s.warmupDone) {
    reason = `ts-lsp: warm up the LSP first — call the LSP tool on ${file} (jump to definition / find references), then re-read.`;
  } else if (s.readCount >= GATE2_AT && s.navCount < 2) {
    // Gate 2: >=4 reads and fewer than 2 LSP calls -> require surgical mode (2 navs)
    reason = `ts-lsp: ${s.readCount} reads with <2 LSP navigation calls. Make one more LSP call (find references / jump to definition) on ${file} to enter surgical mode.`;
  }
