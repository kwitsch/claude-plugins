...
Reflects on this session's own work, reports a summary, and saves durable
lessons as memory. Runs **inline** -- never fork this skill (`context: fork`
starts an isolated subagent with no conversation history, and this skill's
entire job depends on that history).
...
3. **Write.** If `feedback_self_improvement_efficiency.md` already
   exists, back it up alongside itself first (copy it to
   `feedback_self_improvement_efficiency.md.bak`) then write the merged
   content. If it doesn't exist yet, author it fresh via this session's
   own auto-memory two-step save process: frontmatter `name:
   ...
   **How to apply:** lines. State plainly in the body that these are
   self-derived retrospective findings from Claude's own analysis of a
   session, not user-issued corrections -- so a future session doesn't
   misattribute them as an explicit user instruction.
4. **Index.** Add the one-line `MEMORY.md` pointer for the file if it was
   ...
   already existed and only its content changed.
