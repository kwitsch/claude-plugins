...
| coding-toolbox:fresh-work | implementer; reviewer; fixer (wave-parallel per `references/implementing.md`'s Parallelism analysis — Agent engine batches a wave's implementers, then its reviewers, then its fixers, each in one message; size-1 waves stay one at a time; merge-back runs via the orchestrator's own Bash, not a dispatch; the Workflow engine gates internally) | reviewer-batch dispatch; fixer-batch dispatch; next-wave dispatch | medium |
