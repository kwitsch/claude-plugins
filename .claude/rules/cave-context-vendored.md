...
3. **Strip — keep only what Claude Code needs at runtime.** Remove:
   ...
   - **`start.mjs`, `scripts/`, `bin/`, `README.md`** — npm-package launcher / install-heal scripts / CLI shim / upstream docs (none run: import + direct hook-script spawn only).
4. **Keep:** `server.bundle.mjs`, `cli.bundle.mjs` (ctx_insight's upgrade-hint path), `hooks/` (the Claude Code hook scripts + `hooks/core/`, `hooks/formatters/`, the `*.bundle.mjs` work modules + shared `*.mjs`), `insight/` (ctx_insight dashboard, spawned as `node insight/server.mjs`), `configs/claude-code`, `package.json` (provenance), `LICENSE`, `NOTICE`.
