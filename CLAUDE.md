...
- `src/<name>/` — TypeScript sources whose build output is a **committed** plugin artifact; `universal-format` only (`src/universal-format-mcp/` → `plugins/universal-format/mcp/server.mjs` **plus the two committed `.wasm` sidecars `web-tree-sitter.wasm` and `tree-sitter-java_orchard.wasm` in that same directory**, bundled by `bun build` and copied by `build.mjs`). The repo-wide convention stays hand-written zero-dep `.mjs`.
