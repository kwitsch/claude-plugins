...
process.env.CLAUDE_PLUGIN_DATA = mkdtempSync(join(tmpdir(), 'shelllsp-'));
const { freshState, readState, writeState, resetState, statePath } = await import('../../plugins/shell-lsp/mcp/state.mjs');
...
test('writeState fails open when the data dir cannot be created', () => {
  ...
  const f = join(mkdtempSync(join(tmpdir(), 'shelllsp-f-')), 'not-a-dir');
  writeFileSync(f, 'x');
