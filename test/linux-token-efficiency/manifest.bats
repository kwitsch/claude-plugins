...
@test "plugin.json is valid JSON with name and version 0.6.1" {
  run jq -e '.name == "linux-token-efficiency" and .version == "0.6.1"' "$MANIFEST"
  assert_success
}
...
@test "plugin.json declares exactly auto_rewrite, cbm_enabled, rtk_enabled and steer_enabled boolean toggles, all default true" {
  run jq -e '.userConfig | keys == ["auto_rewrite","cbm_enabled","rtk_enabled","steer_enabled"]' "$MANIFEST"
  assert_success
  ...
  assert_success
  run jq -e '.userConfig | .rtk_enabled.type == "boolean" and .rtk_enabled.default == true and (.rtk_enabled.title | length > 0) and (.rtk_enabled.description | length > 0)' "$MANIFEST"
  assert_success
