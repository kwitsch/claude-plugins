...
@test ".mcp.json launches the ts-lsp-hooks server via the mjs-launch.sh wrapper" {
  run jq -e '.mcpServers["ts-lsp-hooks"] | (.command | endswith("bin/mjs-launch.sh")) and (.args[0] | endswith("mcp/server.mjs"))' "$PLUGIN/.mcp.json"
  [ "$status" -eq 0 ]
}
