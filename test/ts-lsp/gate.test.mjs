...
process.env.TS_LSP_ENFORCE_SEARCH = 'true';
process.env.TS_LSP_ENFORCE_READ_GATE = 'true';
const H = await import('../../plugins/ts-lsp/mcp/handlers.mjs');
const { resetState, readState } = await import('../../plugins/ts-lsp/mcp/state.mjs');
...
test('search guard denies TS code-symbol grep', () => {
  const o = H.handlePreToolUse({ tool_name: 'Grep', tool_input: { pattern: 'getUserById', glob: '**/*.ts' }, cwd: CWD });
  assert.equal(isDeny(o), true);
});
test('search guard passes ambiguous (no TS target)', () => {
  const o = H.handlePreToolUse({ tool_name: 'Grep', tool_input: { pattern: 'getUserById' }, cwd: CWD });
  assert.deepEqual(o, {});
});
test('search guard passes non-symbol TS grep', () => {
  const o = H.handlePreToolUse({ tool_name: 'Grep', tool_input: { pattern: 'TODO', glob: '**/*.ts' }, cwd: CWD });
  assert.deepEqual(o, {});
});
test('Gate 1: first TS read is blocked until warmup', () => {
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true);
});
test('non-TS read is never gated', () => {
...
test('after 1 LSP call, reads 1-3 pass; gate 2 blocks read 4 until 2 LSP calls', () => {
  ...
  for (let i = 0; i < 3; i++) assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // read #4, navCount=1 < 2 -> gate 2 fires
});
test('surgical mode: 2 LSP calls unlock unlimited reads', () => {
  H.handlePostToolUse(lsp()); H.handlePostToolUse(lsp()); // navCount=2
  for (let i = 0; i < 10; i++) assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});
});
test('escape hatch: blocked reads with zero LSP release after ESCAPE_THRESHOLD', () => {
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // blockedNoNav=1
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // blockedNoNav=2 -> sets lspUnavailable
  assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});       // now fail-open
});
test('post-warmup escape hatch: 1 LSP call then ESCAPE_THRESHOLD blocked reads release (F1)', () => {
  H.handlePostToolUse(lsp());                                         // navCount=1, warmupDone
  for (let i = 0; i < 3; i++) assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {}); // reads 1-3 pass
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // read 4: gate 2, blockedNoNav=1
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // read 5: gate 2, blockedNoNav=2
  assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});       // read 6: escape hatch -> fail-open
});
test('first-sighting reset wipes inherited surgical mode once per process', () => {
  ...
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // reset re-armed Gate 1
});
...
test('search guard denies Grep alternation pattern on TS target (I1 regression)', () => {
  const o = H.handlePreToolUse({ tool_name: 'Grep', tool_input: { pattern: 'getUserById|createOrder', glob: '**/*.ts' }, cwd: CWD });
  assert.equal(isDeny(o), true);
});

test('search guard denies Glob with TS symbol token', () => {
  const o = H.handlePreToolUse({ tool_name: 'Glob', tool_input: { pattern: '**/*Service*.ts' }, cwd: CWD });
  assert.equal(isDeny(o), true);
});
test('search guard passes Glob with non-TS extension', () => {
  const o = H.handlePreToolUse({ tool_name: 'Glob', tool_input: { pattern: '**/*Service*.go' }, cwd: CWD });
  assert.deepEqual(o, {});
});

test('search guard denies Bash grep with TS symbol on TS file', () => {
  const o = H.handlePreToolUse({ tool_name: 'Bash', tool_input: { command: 'grep getUserById src/app.ts' }, cwd: CWD });
  assert.equal(isDeny(o), true);
});
test('search guard passes Bash grep with TS symbol on non-TS file', () => {
...
test('PostToolUse LSP success re-arms the read gate (one nav re-gates, two enter surgical mode)', () => {
  ...
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // readCount=1, blockedNoNav=1
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // readCount=2, blockedNoNav=2 -> lspUnavailable=true
  assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});       // lspUnavailable: fail-open
  ...
  assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});       // readCount=3 (<GATE2_AT): pass
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // readCount=4 (>=GATE2_AT), navCount=1<2 -> gate 2 re-fires

  // A SECOND successful LSP call (navCount=2) enters surgical mode -> reads pass.
  H.handlePostToolUse(lsp());                                          // navCount=2 -> surgical mode
  for (let i = 0; i < 5; i++) assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});
});
...
test('PostToolUse failed LSP (success:false) does NOT re-arm: navCount stays 0, escape hatch holds (F4)', () => {
  ...
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // blockedNoNav=1
  assert.equal(isDeny(H.handlePreToolUse(read('/proj/a.ts'))), true); // blockedNoNav=2
  assert.deepEqual(H.handlePreToolUse(read('/proj/a.ts')), {});       // entry check -> lspUnavailable=true, fail-open
