...
| [superpowers-automation](plugins/superpowers-automation/README.md) | Classifies a work description as feature/fix/refactor and runs the matching superpowers pipeline (feature/refactor: brainstorm → plan → implement; fix: systematic-debugging), plus a file-advisor-improver reviser skill and an opt-in hook forcing Subagent-Driven implementation.          |
...
| [coding-toolbox](plugins/coding-toolbox/README.md)                 | Enforces the Interaction axis and blocks non-UTF-8 file edits automatically, plus fresh-branch, fresh-pr, fresh-work, feature-development, debugging, bump-version and user-level rules-setup skills for a self-contained git/PR workflow.                                                    |
| [universal-format](plugins/universal-format/README.md)             | Auto-formats just-written Shell/Java/Kotlin/JS-TS/Python/Go/JSON/YAML/Markdown files after Write/Edit using each language's standard formatter, honoring `.editorconfig` and tool-native configs.                                                                                             |
| [universal-lint](plugins/universal-lint/README.md)                 | Runs each language's standard linter (read-only) on just-written Shell/Java/Kotlin/JS-TS/Python/Go/YAML/Markdown/SCSS files after Write/Edit, surfacing findings for Claude to fix; TypeScript files also get a whole-project `tsc --noEmit` type-check.                                      |
