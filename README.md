...
| [universal-format](plugins/universal-format/README.md) | Silently auto-formats just-written Shell/Java/Kotlin/JS-TS/Python/Go files after Write/Edit via each language's standard formatter (no-op when the formatter is absent), honoring `.editorconfig` and tool-native configs. |
