...
| [linux-token-efficiency](plugins/linux-token-efficiency/README.md) | LINUX ONLY: bundles the rtk token-optimizing CLI proxy as a committed Linux x86_64 binary (on the Bash `PATH` automatically) and rewrites every Bash command through it via a `PreToolUse` hook; toggle `auto_rewrite`, plus a repo-level skill that keeps the bundled binary current.                                      |
