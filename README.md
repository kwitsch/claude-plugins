...
| [universal-format](plugins/universal-format/README.md)           | Formats prettier languages (JS/TS/JSON/YAML/Markdown/CSS/SCSS) in-process before the write when an importable prettier is available (else on disk after it) and Shell/Java/Kotlin/Python/Go/PHP after it, honoring `.editorconfig`, `.prettierignore`/`.gitignore` and tool-native configs.   |
