...
| [universal-format](plugins/universal-format/README.md) | Silently auto-formats just-written Shell/Java/Kotlin/JS-TS/Python/Go files after Write/Edit via each language's standard formatter (prettier/biome also fall back to `npx`; no-op when no formatter is available), honoring `.editorconfig` and tool-native configs. |
